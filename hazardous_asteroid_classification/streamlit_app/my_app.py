...
H = form.number_input("Absolute magnitude",
                      format="%0.15f",
                      min_value=0.0,
...
diameter = form.number_input("Object diameter (equivalent to a sphere) [Km]",
                             format="%0.15f",
                             min_value=0.0,
...
albedo = form.number_input("Geometric albedo",
                           format="%0.15f",
                           min_value=0.0,
...
i = form.number_input("Inclination (Angle relative to the x-y ecliptic plane) [Degrees]", 
                      format="%0.15f",
                      max_value=360.0,
                      min_value=0.0,
                      help="""# Inclination
...
om = form.number_input("Longitude of the ascending node [Degrees]",
                       format="%0.15f",
                       max_value=360.0,
                       min_value=0.0,
...
w = form.number_input("Argument of perihelion [Degrees]",
                      format="%0.15f",
                      max_value=360.0,
                      min_value=0.0,
...
ma = form.number_input("Mean anomaly [Degrees]",
                       format="%0.15f",
                       max_value=360.0,
                       min_value=0.0,
                       help="""# Mean anomaly
...
n = form.number_input("Mean motion [Degrees/Days]",
                      format="%0.15f",
                      min_value=0.0,
                      help="""# Mean motion
...
moid = form.number_input("Minimum orbit intersection distance with Earth [AU]",
                         format="%0.15f",
                         min_value=0.0,
...
submitted = form.form_submit_button(label="Classify it", type="secondary")
...
if submitted:
    ...
    for x, param in enumerate([i, om, w, ma]):
        if not (param >= 0 and param <= 360):
            form.error(f'The parameter \"{vars[x]}\" must be greater than or equal to 0 and less than or equal to 360.', icon="🚨")
            error = True

    vars = ["Absolute magnitude", "Mean motion", "Minimum orbit intersection distance with Earth"]
    for x, param in enumerate([H, n, moid]):
        if not param > 0:
            form.error(f'The parameter \"{vars[x]}\" must be greater than to 0.', icon="🚨")
            error = True
    
    vars = ["Object diameter", "Geometric albedo"]
    for x, param in enumerate([diameter, albedo]):
        if not param >= 0:
            form.error(f'The parameter \"{vars[x]}\" must be greater than or equal to 0.', icon="🚨")
            error = True
    ...
    if error == False:       
        ...
        if pred == 'Y':
            form.warning("This asteroid is potentially dangerous! Get out of here, my friend, run!", icon="😬")
        else:
            form.success("You're in lucky! This is a friendly asteroid.", icon="😀")
